...
func (app *application) routes() http.Handler {
	...
	router.Handler(http.MethodGet, "/user/login", app.sessionManager.LoadAndSave(noSurf(http.HandlerFunc(app.userLogin))))
	router.Handler(http.MethodPost, "/user/login", app.sessionManager.LoadAndSave(noSurf(http.HandlerFunc(app.userLoginPost))))
	router.Handler(http.MethodPost, "/user/logout", app.sessionManager.LoadAndSave(noSurf(app.requireAuthentication(http.HandlerFunc(app.userLogoutPost)))))
